...
import org.thymeleaf.extras.idea.exception.ShouldNotHappenException;
...
class HbParsing {
    ...
    private boolean parseStatement(PsiBuilder builder) {
        ...
        if (tokenType == HbTokenTypes.OPEN || tokenType == HbTokenTypes.OPEN_UNESCAPED) {
        ...
        if (tokenType == HbTokenTypes.COMMENT) {
            parseLeafToken(builder, HbTokenTypes.COMMENT);
            return true;
        }
        ...
        if (tokenType == HbTokenTypes.UNCLOSED_COMMENT) {
            ...
            parseLeafToken(builder, HbTokenTypes.UNCLOSED_COMMENT);
    ...
    private boolean parseOpenBlock(PsiBuilder builder) {
        ...
        if (!parseLeafToken(builder, HbTokenTypes.OPEN_BLOCK)) {
            openBlockStacheMarker.drop();
            return false;
        }

        if (parseInMustache(builder)) {
            parseLeafTokenGreedy(builder, HbTokenTypes.CLOSE);
        }
    ...
    private boolean parseOpenInverse(PsiBuilder builder) {
        ...
        if (!parseLeafToken(builder, HbTokenTypes.OPEN_INVERSE)) {
            // didn't find a standard open inverse token,
            // check for the "{{else" version
            regularInverseMarker.rollbackTo();
            if (!parseLeafToken(builder, HbTokenTypes.OPEN)
                    || !parseLeafToken(builder, HbTokenTypes.ELSE)) {
    ...
    private boolean parseCloseBlock(PsiBuilder builder) {
        ...
        if (!parseLeafToken(builder, HbTokenTypes.OPEN_ENDBLOCK)) {
            closeBlockMarker.drop();
            return false;
        }

        if(parsePath(builder)) {
            parseLeafToken(builder, HbTokenTypes.CLOSE);
        }
    ...
    private void parseMustache(PsiBuilder builder) {
        ...
        if (builder.getTokenType() == HbTokenTypes.OPEN) {
            parseLeafToken(builder, HbTokenTypes.OPEN);
        } else if (builder.getTokenType() == HbTokenTypes.OPEN_UNESCAPED) {
            parseLeafToken(builder, HbTokenTypes.OPEN_UNESCAPED);
        } else {
    ...
    private void parsePartial(PsiBuilder builder) {
        ...
        parseLeafToken(builder, HbTokenTypes.OPEN_PARTIAL);

        parseLeafToken(builder, HbTokenTypes.PARTIAL_NAME);
    ...
    private boolean parseInMustache(PsiBuilder builder) {
        ...
        if (!parsePath(builder)) {
            ...
            if (builder.getTokenType() == HbTokenTypes.DATA_PREFIX
                    && parseLeafToken(builder, HbTokenTypes.DATA_PREFIX)
    ...
    private boolean parseParam(PsiBuilder builder) {
        ...
        if (parseLeafToken(builder, HbTokenTypes.STRING)) {
            stringMarker.drop();
            paramMarker.done(HbTokenTypes.PARAM);
            return true;
        } else {
        ...
        if (parseLeafToken(builder, HbTokenTypes.INTEGER)) {
            integerMarker.drop();
            paramMarker.done(HbTokenTypes.PARAM);
            return true;
        } else {
        ...
        if (parseLeafToken(builder, HbTokenTypes.BOOLEAN)) {
            booleanMarker.drop();
            paramMarker.done(HbTokenTypes.PARAM);
            return true;
        } else {
        ...
        if (parseLeafToken(builder, HbTokenTypes.DATA_PREFIX) && parseLeafToken(builder, HbTokenTypes.DATA)) {
            dataMarker.drop();
            paramMarker.done(HbTokenTypes.PARAM);
            return true;
        } else {
    ...
    private boolean parseHashSegment(PsiBuilder builder) {
        return parseLeafToken(builder, HbTokenTypes.ID)
                && parseLeafToken(builder, HbTokenTypes.EQUALS)
                && parseParam(builder);
    ...
    private boolean parsePathSegments(PsiBuilder builder) {
        ...
        if (!parseLeafToken(builder, HbTokenTypes.ID)) {
    ...
    private void parsePathSegmentsPrime(PsiBuilder builder) {
        ...
        if (!parseLeafToken(builder, HbTokenTypes.SEP)) {
            // the epsilon case
    ...
    private boolean parseLeafToken(PsiBuilder builder, IElementType leafTokenType) {
        ...
        if (builder.getTokenType() == leafTokenType) {
            ...
            return true;
        } else if (builder.getTokenType() == HbTokenTypes.INVALID) {
            while (!builder.eof() && builder.getTokenType() == HbTokenTypes.INVALID) {
                builder.advanceLexer();
            }
            recordLeafTokenError(HbTokenTypes.INVALID, leafTokenMark);
            return false;
        } else {
    ...
    private boolean atOpenInverseExpression(PsiBuilder builder) {
        ...
        if (builder.getTokenType() == HbTokenTypes.OPEN) {
            builder.advanceLexer();
            if (builder.getTokenType() == HbTokenTypes.ELSE) {
                atOpenInverse = true;
            }
        }
